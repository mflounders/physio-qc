function pulse = read_PULS_log_fileSYNC(pulsFile, dicomStart, dicomEnd)
...
%
%   Written by Andrew S Bock and Marta Vidorreta D�az de Cerio Dec 2015

%% Load the file, read the character information
if exist(pulsFile,'file')
    fid = fopen(pulsFile);
    ...
    for i = 1:Nvalues
        ...
        if (b==3)
            ...
            IsTrigger(i) = 0;
        elseif (b==4)
            pulse.data(i) = a(end - length('PULS_TRIGGER')+2);%SC
            pulse.AT_ms(i) = 2.5*a(1);
            IsTrigger(i) = 1;
        end
    end
    ...
    %sync to Dicom data
    pulse.data = pulse.data(pulse.AT_ms >= dicomStart & pulse.AT_ms <= dicomEnd);
    IsTrigger = IsTrigger(pulse.AT_ms >= dicomStart & pulse.AT_ms <= dicomEnd);
    pulse.AT_ms = pulse.AT_ms(pulse.AT_ms >= dicomStart & pulse.AT_ms <= dicomEnd);
