function [output, pulse, resp] = PulseResp(dicomDir, pulsFile,outDir, dicomStart, dicomEnd)
%
%   Usage:
...
for t = 1:length(dicoms)
    dicom.info = dicominfo(fullfile(dicomDir,dicoms(t).name));
...
switch ext
    case '.log'
        pulse = read_PULS_log_fileSYNC(pulsFile, dicomStart, dicomEnd);
    case '.puls'
        pulse = read_PMU_file(pulsFile);
end

%%Pull out resp information from resp file
respFile = sprintf('%sRESP.log',pulsFile(1:end-8));
parts = strsplit(respFile,'pulsdir');
respFile = sprintf('%sPhysio%s', parts{1}, parts{2});
resp = read_PULS_log_fileSYNC(respFile, dicomStart, dicomEnd);


%% Apply smoothing/filtering to the signal
% Filter signal for high frequency peaks following Verstynen & Deshpande (2011)
pulse.Hsignal = filter_signalNODISP(pulse.data_dmean,'band',pulse.sampTsecs,0.6,2);% 0.6-2.0 Hz Butterworth filter;
% Filter signal for low frequency peaks
pulse.Lsignal = filter_signalNODISP(pulse.data_dmean,'low',pulse.sampTsecs,-inf,0.6);
...
output.Highrate = length(find(output.Hevents)) / Time(end) * 60; %Heart rate
output.Lowrate = length(find(output.Levents)) / Time(end) * 60; %Respiration rate
